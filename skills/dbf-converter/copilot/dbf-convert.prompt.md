...
2. **Goal** — one of: "explore", "analysis", "LLM input", "DB ingestion". Infer format from goal:
   - explore → `jsonl` with `--head 100 --schema`
   - analysis → `csv`
   - LLM input → `jsonl --head 500 --schema`
   - DB ingestion → `sql` (ask for table name)
3. **Encoding** — default to `cp850`; ask only if the user hints at non-Brazilian origin.
...
# Pipe straight to PostgreSQL
dbf-converter -i mov.dbf -o /dev/stdout -f sql --table movimento \
  | psql "$DATABASE_URL"

# Non-Brazilian encoding
